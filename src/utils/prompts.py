...
1. **提取时间信息**：
   ...
   - 将时间信息规则化表示，小时统一用h表示，分钟用m表示，秒用s表示。
     比如:1小时20秒表示为["1h20s"]，2小时30分钟表示为["2h30m"]，1分15.5秒到2分20秒表示为["1m15.5s", "2m20s"]。
   - 如果用户查询中未提及时间信息，则返回空列表。
   - 如果是时间点返回格式为[time_point]，如果是时间段返回格式为[start_time, end_time]。
2. **语义检索判断**：
   - 如果用户查询中的时间信息明确且足以通过时间检索获取结果，则标记不需要语义检索。
   - 如果查询中的时间信息不足或查询涉及复杂概念和多个实体，则标记为需要语义检索。
